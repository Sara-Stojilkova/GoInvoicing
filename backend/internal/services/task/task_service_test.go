...
func TestCreateTask(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newTaskService()
			task, err := svc.Create(ctx, tt.title, tt.priority, tt.agencyID, uuid.New(), tt.description, tt.asigneeId, tt.dueDate, nil)
			if err != nil {
...
func TestAssignTask(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyA, uuid.New(), nil, nil, nil, nil)
				return task.ID
			},
...
func TestUnassignTask(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), nil, &assigneeID, nil, nil)
				return task.ID
			},
			wantErr: nil,
		},
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), nil, nil, nil, nil)
				return task.ID
			},
			wantErr: nil,
		},
...
func TestListByAgency(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(svc *services.TaskService) {
				svc.Create(ctx, "Task 1", "high", agencyA, uuid.New(), nil, nil, nil, nil)
				svc.Create(ctx, "Task 2", "medium", agencyA, uuid.New(), nil, nil, nil, nil)
				svc.Create(ctx, "Task 3", "low", agencyB, uuid.New(), nil, nil, nil, nil)
			},
			agencyID:  agencyA,
			wantCount: 2,
		},
...
func TestListOverdue(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(svc *services.TaskService) {
				svc.Create(ctx, "Overdue task", "high", agencyA, uuid.New(), nil, nil, &past, nil)
			},
			agencyID:  agencyA,
			wantCount: 1,
		},
		{
			...
			setup: func(svc *services.TaskService) {
				svc.Create(ctx, "Future task", "low", agencyA, uuid.New(), nil, nil, &future, nil)
			},
			agencyID:  agencyA,
			wantCount: 0,
		},
		{
			...
			setup: func(svc *services.TaskService) {
				task, _ := svc.Create(ctx, "Done task", "medium", agencyA, uuid.New(), nil, nil, &past, nil)
				svc.CompleteTask(ctx, task.ID, now)
			},
			agencyID:  agencyA,
			wantCount: 0,
		},
		{
			...
			setup: func(svc *services.TaskService) {
				svc.Create(ctx, "Agency A task", "high", agencyA, uuid.New(), nil, nil, &past, nil)
				svc.Create(ctx, "Agency B task", "high", agencyB, uuid.New(), nil, nil, &past, nil)
			},
			agencyID:  agencyA,
			wantCount: 1,
		},
...
func TestUpdateDescription(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), &desc, nil, nil, nil)
				return task.ID
			},
			input: &other,
			want:  &other,
...
func TestSetDueDate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), nil, nil, &date, nil)
				return task.ID
			},
			dueDate:     &other,
			wantDueDate: &other,
...
func TestSetInProgress(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), nil, nil, &past, nil)
				svc.CompleteTask(ctx, task.ID, now)
				return task.ID
			},
			wantStatus: "in_progress",
...
func TestUpdateTags(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "clears tags by passing empty slice",
			setup: func(svc *services.TaskService) uuid.UUID {
				task, _ := svc.Create(ctx, "Fix bug", "high", agencyID, uuid.New(), nil, nil, nil, []string{"old"})
				return task.ID
			},
			tags:    []string{},
			wantErr: nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			svc := newTaskService()
			taskID := tt.setup(svc)
			err := svc.UpdateTags(ctx, taskID, tt.tags)
			if !errors.Is(err, tt.wantErr) {
				t.Errorf("UpdateTags() error = %v, want %v", err, tt.wantErr)
			}
			if tt.wantErr != nil {
				return
			}
			...
			if len(task.Tags) != len(tt.tags) {
				t.Errorf("Tags = %v, want %v", task.Tags, tt.tags)
			}
			for i := range tt.tags {
				if task.Tags[i] != tt.tags[i] {
					t.Errorf("Tags[%d] = %q, want %q", i, task.Tags[i], tt.tags[i])
				}
			}
		})
	}
